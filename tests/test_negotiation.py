...
class TestInstrumentValueCalculation:
    ...
    def test_credit_card_value_calculation(self):
        ...
        assert (
            result.value_score > 0.6
        )  # Should be high due to rewards (adjusted for actual calculation)
...
class TestCounterNegotiation:
    ...
    async def test_bnpl_selected_for_flexibility(self):
        ...
        assert (
            response.selected_instrument.preference_score
            > response.rejected_instruments[0].preference_score
        )
...
class TestExplanationGeneration:
    ...
    def test_explanation_includes_counter_proposal(self):
        ...
        explanation = generate_consumer_explanation(credit_card, [], merchant_proposal, request)

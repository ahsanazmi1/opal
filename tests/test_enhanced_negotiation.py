...
class TestMultiInstrumentNegotiator:
    ...
    def test_instrument_value_calculation_deterministic(
        ...
        for result in results[1:]:
            assert abs(result[0] - first_result[0]) < 0.001  # Allow small floating point differences
    ...
    def test_counter_negotiation_deterministic(
        ...
        first_result = results[0]
        for result in results[1:]:
            assert result.selected_instrument.instrument_id == first_result.selected_instrument.instrument_id
    ...
    def test_explainability_diffs(
        ...
        preference_sets = [
            {"cash_rewards": 2.0, "loyalty_programs": 0.5, "bnpl_preference": 1.0},  # Cashback focused
        ...
        preference_mentioned = any(
            "cashback" in exp["explanation"].lower() or 
            "reward" in exp["explanation"].lower() or
            "bnpl" in exp["explanation"].lower()
    ...
    def test_selection_factors_generation(
        ...
        assert any(keyword in factors_text for keyword in ["reward", "cost", "convenient", "loyalty"])
    ...
    def test_golden_fixture_consistency(self, negotiator):
        ...
        if golden_input.exists() and golden_output.exists():
            ...
            instruments = [ConsumerInstrument(**inst) for inst in input_data["available_instruments"]]
            ...
            assert response.selected_instrument.instrument_type == expected_output["selected_instrument_type"]

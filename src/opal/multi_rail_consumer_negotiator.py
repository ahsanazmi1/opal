    ...
    def get_preferred_combinations(self) -> List[Tuple[RailType, InstrumentType]]:
        ...
        for rail_type, rail_info in self.available_rails.items():
            if rail_info["preferred"] and rail_info["available"]:
                for instrument in rail_info["supported_instruments"]:
                    if (
                        instrument in self.available_instruments
                        and self.available_instruments[instrument]["preferred"]
                        and self.available_instruments[instrument]["available"]
                    ):
                        combinations.append((rail_type, instrument))
        return combinations
...
class MultiRailConsumerEvaluator:
    ...
    def _initialize_default_capabilities(self) -> None:
        ...
        self.capability.add_instrument_capability(
            "credit_card", 25000.0, reward_rate=0.015, preferred=True
        )
        ...
        self.capability.add_instrument_capability(
            "stablecoin", 50000.0, reward_rate=0.02, preferred=True
        )
        ...
        self.capability.add_rail_capability(
            "Card", ["credit_card", "debit_card", "prepaid_card"], consumer_benefit=0.02
        )
        self.capability.add_rail_capability(
            "ACH", ["debit_card", "bank_transfer"], consumer_benefit=0.0
        )
        self.capability.add_rail_capability(
            "RTP", ["debit_card", "bank_transfer"], consumer_benefit=0.005, preferred=True
        )
        self.capability.add_rail_capability(
            "FedNow", ["debit_card", "bank_transfer"], consumer_benefit=0.01, preferred=True
        )
        self.capability.add_rail_capability(
            "SEPA", ["bank_transfer", "debit_card"], consumer_benefit=0.0
        )
        self.capability.add_rail_capability(
            "Crypto", ["stablecoin"], consumer_benefit=0.03, preferred=True
        )
    ...
    def evaluate_consumer_combination(
        ...
        convenience_score = self._calculate_convenience_score(
            rail_type, instrument_type, consumer_context
        )
        cost_savings = self._calculate_cost_savings(
            merchant_proposal, rail_type, transaction_amount
        )
        ...
        confidence = self._calculate_consumer_confidence(
            instrument_info, rail_info, consumer_context
        )
    ...
    def _calculate_reward_value(
        self, instrument_info: Dict[str, Any], transaction_amount: float
    ) -> float:
    ...
    def _calculate_convenience_score(
        self, rail_type: RailType, instrument_type: InstrumentType, consumer_context: Dict[str, Any]
    ) -> float:
    ...
    def _calculate_cost_savings(
        self, merchant_proposal: MerchantProposal, rail_type: RailType, transaction_amount: float
    ) -> float:
    ...
    def _calculate_preference_bonus(
        self, instrument_info: Dict[str, Any], rail_info: Dict[str, Any]
    ) -> float:
    ...
    def _generate_consumer_explanation(
        ...
        explanation_parts.append(
            f"{instrument_type.replace('_', ' ').title()} via {rail_type} rail"
        )
...
class MultiRailConsumerNegotiator:
    ...
    def _create_fallback_response(
        self, merchant_proposal: MerchantProposal, transaction_amount: float
    ) -> CounterNegotiationResponse:
    ...
    def _create_consumer_proposal(
        self, evaluation: Dict[str, Any], transaction_amount: float
    ) -> ConsumerProposal:
    ...
    def _generate_negotiation_explanation(
        ...
        # Optimal choice
        explanation_parts.append(
            f"Selected {optimal['instrument_type'].replace('_', ' ').title()} via {optimal['rail_type']} rail"
        )
        ...
        if alternatives:
            ...
            explanation_parts.append(
                f"Alternative: {second_best['instrument_type'].replace('_', ' ').title()}/{second_best['rail_type']} "

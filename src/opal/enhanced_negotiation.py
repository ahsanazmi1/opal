...
class MultiInstrumentNegotiator:
    ...
    def calculate_instrument_value(
        ...
        weights = self.instrument_weights.get(instrument_type, {"rewards": 0.33, "cost": 0.33, "convenience": 0.34})
    ...
    def counter_negotiate(
        ...
        merchant_savings = self._calculate_merchant_savings(optimal_instrument, request.merchant_proposal)
        ...
        logger.info(f"Enhanced counter-negotiation completed. Selected {optimal_instrument.instrument_type} with score {optimal_instrument.value_score:.3f}")
    ...
    def _generate_counter_proposal(
        ...
        return {
            ...
            "merchant_benefits": {
                "potential_savings": self._calculate_merchant_savings(instrument, merchant_proposal),
    ...
    def _generate_explanation(
        ...
        explanation_parts.append(f"Selected {instrument_type.replace('_', ' ').title()} from {selected_instrument.provider}")
        ...
        if selected_instrument.total_reward_value > 0:
            explanation_parts.append(f"Earns ${selected_instrument.total_reward_value:.2f} in rewards")
        ...
        if merchant_savings > 0:
            explanation_parts.append(f"Could save merchant ${merchant_savings:.2f} vs current proposal")
        
        # Alternative comparison
        if alternatives:
            best_alt = alternatives[0][0]
            explanation_parts.append(f"Chosen over {best_alt.instrument_type.replace('_', ' ')} (score: {best_alt.value_score:.2f})")

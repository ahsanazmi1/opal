...
from .rail_evaluation import (
    determine_consumer_rail_preference, 
    ConsumerRailPreferences,
...
async def enhanced_counter_negotiation_with_rail_evaluation(
    ...
    try:
        logger.info(
            f"Starting enhanced counter-negotiation with rail evaluation",
            extra={
        ...
        rail_instruments = [
            inst for inst in request.available_instruments
        ...
        if not rail_instruments:
            ...
            logger.warning(f"No instruments available for preferred rail {consumer_preferred_rail}, using merchant rail {request.merchant_proposal.rail_type}")
        ...
        if rail_instruments:
            ...
            for instrument in rail_instruments:
                try:
                    ...
                    logger.warning(f"ML scoring failed for instrument {instrument.instrument_id}: {e}")
                    ml_value_scores[instrument.instrument_id] = instrument.value_score
                    evaluated_instruments.append(instrument)
            ...
            if evaluated_instruments:
                selected_instrument = max(evaluated_instruments, key=lambda x: x.value_score)
                rejected_instruments = [inst for inst in evaluated_instruments if inst.instrument_id != selected_instrument.instrument_id]
            else:
        ...
        logger.info(
            f"Enhanced counter-negotiation complete",
            extra={
                "actor_id": request.actor_id,

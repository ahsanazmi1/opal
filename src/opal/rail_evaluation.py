...
class ConsumerRailPreferences(BaseModel):
    ...
    reward_weight: float = Field(default=0.4, description="Weight for reward optimization", ge=0.0, le=1.0)
    cost_weight: float = Field(default=0.3, description="Weight for cost minimization", ge=0.0, le=1.0)
    convenience_weight: float = Field(default=0.2, description="Weight for convenience", ge=0.0, le=1.0)
...
def evaluate_rail_for_consumer(
    ...
    cost_efficiency_score = calculate_cost_efficiency_score(
        merchant_proposal, preferences
    )
    
    # Calculate convenience score
    convenience_score = calculate_convenience_score(
        rail_type, rail_characteristics, preferences
    )
    ...
    explanation, key_factors = generate_rail_explanation(
        rail_type, consumer_benefit_score, cost_efficiency_score,
        convenience_score, reward_potential_score, composite_score,
        merchant_proposal, preferences
    )
...
def get_rail_characteristics(rail_type: RailType) -> Dict[str, Any]:
    ...
    return characteristics.get(rail_type, {
        "settlement_speed": 3,
...
def calculate_consumer_benefit_score(
    ...
    rail_instruments = [
        ...
        if instrument_supports_rail(inst, rail_type)
    ]
        ...
        for instrument in rail_instruments:
            try:
                ml_result = score_consumer_instrument_value(
                    rewards_rate=instrument.get("total_reward_value", 0.0) / preferences.transaction_amount,
                ...
                best_instrument_score = max(best_instrument_score, instrument.get("value_score", 0.5))
...
def calculate_cost_efficiency_score(
    ...
    cost_ratio = cost_percentage / (preferences.transaction_amount / 100)  # Ratio of cost to transaction
...
def calculate_reward_potential_score(
    ...
    if rail_instruments:
        max_reward_rate = max(
            (inst.get("total_reward_value", 0.0) / 100.0)  # Assume $100 transaction for rate calculation
...
async def determine_consumer_rail_preference(
    ...
    confidence = min(1.0, max(0.0, preferred_evaluation.composite_score))  # Ensure confidence is 0-1
    ...
    if preferred_rail == merchant_proposal.rail_type:
        ...
        merchant_eval = next(
            (eval for eval in evaluations if eval.rail_type == merchant_proposal.rail_type),
            None
        )
        
        if merchant_eval and (preferred_evaluation.composite_score - merchant_eval.composite_score) > 0.2:
...
def create_counter_proposal(
    ...
    return {
        ...
        "consumer_benefit": evaluation.consumer_benefit_score * preferences.transaction_amount * 0.02,  # Estimate 2% benefit
...
def estimate_merchant_cost_impact(
    ...
    return {
        ...
        "impact": "increase" if cost_difference > 0 else "decrease" if cost_difference < 0 else "neutral"
    }
